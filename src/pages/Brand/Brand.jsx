...
export default function Brand() {
    ...
    async function getProductByBrand() {
        setloader(true)
        await axios.get(`https://ecommerce.routemisr.com/api/v1/products?brand=${id}`)
            .then((res) => {
        ...
        <div className='container'>
            <div>
                <h1 className='text-3xl font-bold text-center my-12'>
                    {brand.name} Products
                    {
                        products.length === 0 && <p className='text-center mt-8 text-red-500'>Oops! No Products Found <br /> in this brand</p>
                    }
                </h1>
            </div>
